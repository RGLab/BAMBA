...
// given by ag and re
// Prior probability of is a product of ag, re, and timepoint probabilities

// y_i = [y_{i1}, ..., y_{iT}]^T
// f(y_{it} | \theta) = N(\mu_{0t}, \sigma_t)^{1-z_{it}} N(\mu_{0t} + mu_{art}, \sigma_t)^{z_{it}}
// z_{it} ~ Bernoulli(\omega_{ag} * \omega_{re} * \omega_t)
...
data {
  ...
  int<lower=1> N_re; // grouping for Fc variable
...
transformed parameters {
  ...
  for (n in 1:N) {
      soft_z[n][1] = log(1-omega_ag[obs_to_ag[n]] *
                             omega_re[obs_to_re[n]] * omega_t[obs_to_t[n]]) +
			normal_lpdf(ystar[n] | mu0[obs_to_t[n]], sigma[obs_to_t[n]]);
      soft_z[n][2] = log(omega_ag[obs_to_ag[n]] *
                           omega_re[obs_to_re[n]] * omega_t[obs_to_t[n]]) +
                    normal_lpdf(ystar[n] | mu0[obs_to_t[n]] +
